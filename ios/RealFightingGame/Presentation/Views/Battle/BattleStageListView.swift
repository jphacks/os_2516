...
struct BattleStageListView: View {
    ...
    @State private var pendingStageID: String? = nil
    @State private var showBattleSheet: Bool = false
    @State private var battleStageIDForBattleView: String? = nil
    ...
    private func stagesSection(_ stages: [BattleStageListViewModel.Stage]) -> some View {
        Section("付近のステージ") {
            ForEach(stages) { stage in
                ...
                Button {
                    // prevent duplicate taps for same stage while pending
                    if let pending = pendingStageID, pending == stage.id {
                        print("[BattleStageListView] join ignored: already joining stage id=\(stage.id)")
                        return
                    }

                    pendingStageID = stage.id
                    battleStageIDForBattleView = stage.id
                    battleServiceForBattleView = service
                    // navigate to BattleView; actual join is performed by BattleViewModel.onAppear
        ...
        // programmatic navigation target for BattleView
        .background(
            NavigationLink(destination: AnyView(
                Group {
                    if let stageId = battleStageIDForBattleView, let svc = battleServiceForBattleView {
                        BattleView(sessionID: stageId, service: svc, motionService: container.motionService, locationService: container.locationService)
                    } else {
                        EmptyView()
                    }
                }
            ), isActive: $showBattleSheet) { EmptyView() }
        )
        .onChange(of: showBattleSheet) { new in
            if !new {
                // clear pending / temporary state on pop
                pendingStageID = nil
                battleStageIDForBattleView = nil
