...
特に、戦闘に勝つためには早いタイミングで切れるMPを回復させながら戦う必要があるが、MPの回復は**早く走るほど早く回復する仕様**のため、自然と**運動量が増える設計**をした。
...
* **CI/CD**: 早い段階でGitHubActionsを活用したCloudRunへのワークフローを組み立てた。

## 開発技術
### 活用した技術
#### API・データ
* **Supabase(PostgreSQL)**: プレイヤー情報、ゲームセッション、位置ログの管理
...
- Cloud Runへのデプロイで権限が足りているはずなのにIAMロールに権限が足りないとずっとでて苦労した。面倒になって最後は強い権限与えた。
